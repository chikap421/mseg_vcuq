...
// the "-4" is to remove the ".jpg" 
shorter_name = substring(filename, 0, lengthOf(filename)-4);	
Raw_img = "" + shorter_name;
print(Raw_img);
open(input_directory+Raw_img+".jpg");

selectWindow(Raw_img+".jpg");
...
selectWindow(Raw_img+".jpg - composite - 32-Bit - rescaled (xy) - normalized - score (segmentation)");
run("32-bit");
selectWindow(Raw_img+".jpg - composite - 32-Bit - rescaled (xy) - normalized");
run("RGB Color");
run("32-bit");
selectWindow(Raw_img+".jpg - composite - 32-Bit - rescaled (xy) - normalized - score (segmentation)");
run("Merge Channels...", "c2=[" + Raw_img + ".jpg - composite - 32-Bit - rescaled (xy) - normalized - score (segmentation)] c4=[" + Raw_img+ ".jpg - composite - 32-Bit - rescaled (xy) - normalized (RGB)] create keep");
